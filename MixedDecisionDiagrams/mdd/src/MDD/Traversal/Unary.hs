...
restrictPassedSameClass :: forall a. (DdF3 a) => UnOpContext -> [Position] -> Bool -> Node -> (UnOpContext, Node)
restrictPassedSameClass c (na : nas) b d =
  let (c', d') = inferNode @a c (last na) d
  in restrict_node_set @a c' (na : nas) b d'
restrictPassedSameClass _ [] _ _ = error "restrictPassedSameClass: empty position list"

-- | 'PassedPreviousLayer' (simple): wrap current root as 'EndClassNode' and recurse.
restrictPassedPreviousLayerSimple :: forall a. (DdF3 a) => UnOpContext -> [Position] -> Bool -> Node -> (UnOpContext, Node)
restrictPassedPreviousLayerSimple c (na : nas) b d =
  let (c1, endclass_wrapped) = insert c (EndClassNode (fst d))
  in restrict_node_set @a c1 (na : nas) b endclass_wrapped
restrictPassedPreviousLayerSimple _ [] _ _ = error "restrictPassedPreviousLayerSimple: empty position list"

-- | 'PassedDeeperLayer': stage, 'inferClassNode' at @infnode_position@, recurse.
restrictPassedDeeperLayer ::
  forall a. (DdF3 a) =>
  UnOpContext ->
  [Position] ->
  Bool ->
  Node ->
  Int ->
  RestrictDeeperLayerStage ->
  (UnOpContext, Node)
restrictPassedDeeperLayer c (na : nas) b d infnode_position stage =
  let (c1, d1) = stage c d
      (c2, d2) = inferClassNode @a c1 infnode_position d1
  in restrict_node_set @a c2 (na : nas) b d2
restrictPassedDeeperLayer _ [] _ _ _ _ = error "restrictPassedDeeperLayer: empty position list"
      ...
      where
        restrict_node_set_internal c (na : nas) b d@(node_id, Node position pos_child neg_child)  =
            ...
            in case pathCmp of
                ...
                RestrictPassed PassedSameClass -> restrictPassedSameClass @a c (na : nas) b d
                RestrictPassed PassedPreviousLayer -> restrictPassedPreviousLayerSimple @a c (na : nas) b d
                RestrictPassed PassedDeeperLayer ->
                    restrictPassedDeeperLayer @a c (na : nas) b d
        ...
        restrict_node_set_internal c (na : nas) b d@(node_id, ClassNode position dc p n) =
            ...
            in if cp > na
                then -- We've passed the target position, need to infer an ClassNode at the target
                    ...
                    c_ = add_to_stack_ (position, Dc) (((0, 0), Unknown), ((0, 0), Unknown)) (traverse_dc_unary @a MvClassDc c node_id)
                    (c1, dcR) = restrict_node_set @Dc c_ (na : nas) b (getNode c dc)
                    (c1', dcR') = absorb_dc_unary @Dc c1 dcR

                    c2_ = add_to_stack_ (position, Neg) (getNode c1' dc, dcR') (traverse_dc_unary @a MvClassNeg (reset_stack_un c1' c) node_id)
                    (c2, nR) = restrict_node_set @Neg c2_ (na : nas) b (getNode c1' n)
                    (c2', nR') = absorb_unary @Neg c2 dcR' nR

                    c3_ = add_to_stack_ (position, Pos) (getNode c2' dc, dcR') (traverse_dc_unary @a MvClassPos (reset_stack_un c2' c) node_id)
    ...
    restrict_node_set c (na : nas) b d@(node_id, EndClassNode child) =
      ...
      where
        restrict_node_set_internal c (na : nas) b d@(node_id, EndClassNode child) =
            ...
            in case infkind of
                PassedSameClass -> restrictPassedSameClass @a c (na : nas) b d
                PassedPreviousLayer ->
                    -- Pop stack, sync dc traversal, recurse into child (not the simple wrap+recurse path)
                    let
                        (c_, inf) = pop_stack_ c
                        c' = traverse_dc_unary @a ExitClass c_ node_id
                        (c'', (r, _)) = case inf of
                ...
                PassedDeeperLayer ->
                    ...
                    in restrictPassedDeeperLayer @a c (na : nas) b d infnode_position wrapCurrentRootAsEndClass
    ...
    restrict_node_set c (na : nas) b d@(_, Leaf _) =
      ...
      where
        restrict_node_set_internal c (na : nas) b d@(_, Leaf _) =
            ...
            in case infkind of
                PassedSameClass -> restrictPassedSameClass @a c (na : nas) b d
                PassedPreviousLayer -> restrictPassedPreviousLayerSimple @a c (na : nas) b d
                PassedDeeperLayer ->
                    let current_lv = current_path
                        infnode_position = na !! length current_lv
                    in restrictPassedDeeperLayer @a c (na : nas) b d infnode_position wrapCurrentRootAsEndClass
        ...
        restrict_node_set_internal c (na : nas) b d@(_, Unknown) =
            ...
            in case infkind of
                ...
                PassedDeeperLayer ->
                    let current_lv = current_path
                        infnode_position = na !! length current_lv
                    in restrictPassedDeeperLayer @a c (na : nas) b d infnode_position restrictDeeperLayerBare
