...
-- (.*.) c a b =
--     let r = debug_func "INTER" $ intersectionLocal @Dc c{func_stack = []} a b (getDd c a) (getDd c b)
...
ddOf c (Or a b) =
                let
                    ...
                    (c2, r2) = ddOf c1 b
                in (.+.) c2 r1 r2
ddOf c (Impl a b) = ddOf c $ Or (Neg a) b
ddOf c (ImplR a b) = ddOf c $ Or a (Neg b)
...
x =                     makeNode c (L [(0, Dc1)] 2)
xx = take_c__ x         makeNode (L [(0, Dc1)] (-2))
x' = take_c_ xx         (-.) x
h = take_c__ x'         makeNode (L [(0, Dc1)] 3)
j = take_c__ h          makeNode (L [(0, Dc1)] (-1))
jj = take_c__ j         makeNode (L [(0, Dc1)] (1))
-- 1    , neg1
-- neg2 , 2
-- 3
