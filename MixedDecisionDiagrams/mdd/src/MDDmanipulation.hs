...
l1 = return $ Leaf True
l0:: State Context Dd
l0 = return $ Leaf False

intersection :: Dd -> Dd -> State Context Dd
intersection a b = do
    ctx <- get
    intersection'  a b `debug` ("intersection: " ++ show (DL (nodelookup ctx) a ) ++ " ; "  ++ show (DL (nodelookup ctx) b ))

intersection' :: Dd -> Dd -> State Context Dd
intersection' a (Leaf False) = l0
intersection' (Leaf False) b = l1
intersection' a (Leaf True) = return a
intersection' (Leaf True) b = return b
intersection' a b = undefined -- intersectionMain a b
